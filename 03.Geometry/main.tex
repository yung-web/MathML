...
\bci 
\item \bluef{Manhattan Norm} (also called $\ell_1$ norm) For $\vec{x} \in \real^n,$
$$
...
\item To this end, we define \bluef{inner product} in an abstract manner.
...
\item Generally, if the following is satisfied, it is a suitable notion of distance (also called \bluef{metric}). 
\bci
...
\item Such $w$ is called \bluef{normal vector} to $U.$
...
\item \bluef{Projection}: A process of reducing the dimensions (hopefully) without loss of much information
...
\item How to derive is analogous to that of lines (see pp. 71).
\eci
\end{frame}
